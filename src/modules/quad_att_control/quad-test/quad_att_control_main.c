...
int att_control_thread_main(int argc, char *argv[]) {
        ...
        while (!thread_should_exit) {
                ...
                int ret_cmd = poll(fd_cmd, 1, 1);
                if (ret_sp < 0) {
                        warnx("poll cmd error");
                } else if (ret_cmd == 0) {
                        /* no return value - nothing has happened */
                } else if (fd_cmd[0].revents & POLLIN) {
                        orb_copy(ORB_ID(quad_att_sp), quad_sp_sub, &sp); /* skal være et nyt cmd topic */
                } else {
                        /* nothing happened */
                }
                ...
                if ( cmd struct == start ) {
                        /* start skidtet */
                } else if ( cmd struct == land) {
                        /* land */
                } else if ( cmd struct == stop) {
                        /* stop skidtet */
                } else if ( cmd struct == emergency ) {
                        /* nødstop */
                } else {
                        /* This is not the line you where loking for */
                }
...
int control(struct *sp) {
        ...
        static struct attError_s error_der;
